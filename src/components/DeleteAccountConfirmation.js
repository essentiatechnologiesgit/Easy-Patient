  ...
  return (
    <Modal isVisible={isVisible} hasBackdrop={true} onBackdropPress={toggleModal} style={{ justifyContent: "center", alignItems: "center" }}>
      <View style={{ backgroundColor: "white", padding: 20, borderRadius: 4, width: "90%" }}>
        <Text style={{ marginBottom: 10, color: config.textColorHeadings, fontWeight: 500, fontSize: PixelRatio.getFontScale() * 20, }}>Easy Patient</Text>
        ...
        <View style={{ paddingTop: 10, flexDirection:'row', gap:14, justifyContent:'flex-end'}}>
          <TouchableWithoutFeedback onPress={toggleModal}>
            <Text style={{ color: config.secondaryColor,fontSize: PixelRatio.getFontScale() * 16 }}>CANCEL</Text>
          </TouchableWithoutFeedback>
          <TouchableWithoutFeedback onPress={Confirm}>
            <Text style={{ color: config.secondaryColor,fontSize: PixelRatio.getFontScale() * 16}}>SWITCH OFF</Text>
          </TouchableWithoutFeedback>
        </View>
      </View>
    </Modal>
